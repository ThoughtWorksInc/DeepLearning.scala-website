...
Since Scala `trait`s can be nested, we only refer the root level `trait` as a plugin. For example, `Plugin1#INDArrayLayerApi` is a mixin trait but not a plugin; `Plugin1` is the plugin. Plugin users only create hyperparameters object from plugins (root level mixins), then all the nested types in those plugins are got mixed-in automatically.
